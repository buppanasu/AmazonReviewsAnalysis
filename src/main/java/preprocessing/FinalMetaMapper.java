...
public class FinalMetaMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        try {
            ...
            String features = product.has("features") ? product.get("features").toString() : "";
            String description = product.has("description") ? product.get("description").toString() : "";
            float price = product.has("price") ? product.get("price").floatValue() : 0.0f;
            int imageCount = product.has("images") ? product.get("images").size() : 0;
            int videoCount = product.has("videos") ? product.get("videos").size() : 0;
            String store = product.has("store") ? product.get("store").asText("") : "";
            String details = product.has("details") ? product.get("details").toString() : "";
            ...
            outputValue.put("title", title);
            outputValue.put("average_rating", averageRating);
            outputValue.put("rating_number", ratingNumber);
            outputValue.put("features", features);
            outputValue.put("description", description);
            outputValue.put("price", price);
            outputValue.put("image_count", imageCount);
            outputValue.put("video_count", videoCount);
            outputValue.put("store", store);
            outputValue.put("details", details);
